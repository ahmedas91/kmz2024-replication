...
the paper's variable-importance ranking, and extends the analysis five ways:
updated data through 2024, government and corporate bonds, developed ex-US
equities, the source-faithful counterfactuals of
\citet{nagel2025seemingly}, and matched markets with an explicitly planted
nonlinear rule. The virtue of complexity replicates and survives updated data
and bond markets but does not survive the trip abroad. The final experiments
favor Nagel's mechanical account of the historical payoff, while the matched
twins show that the estimator can recover a strong planted nonlinear signal in
a clean design. Evidence for an economically distinct payoff, however,
disappears at realistic signal strength and weakens when each world receives
its own lagged return.
\end{abstract}
...
equities, implement the direct tests of \citet{nagel2025seemingly}, and add
matched +/- markets that distinguish payoff spanning from recovery of a
planted conditional mean. Sections~\ref{sec:data}
